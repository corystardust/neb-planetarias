...
star=[19.106,21,11,11,9.1 ,20.2,11   ,9.1 ,11   ,9.1  ]   
...
for nlin in range(220,228):
  ...
  npix= pixeles[dn]
  mapa = np.zeros((nrb,npix), dtype=float)
  for nr in range(nrb):
    ...
    mapa[nr,:]=espectro['flux'][nol]
  w=espectro['wavelen'][nol]
  ...
  if i+sh >= 2048:
      ...
      bd=i+sh
  #Para la prueba
  #filename = 'hf2cd4r.fits'
  #imf = pyfits.PrimaryHDU()
  #imf.data = np.arange(nrb, sh * 2)
  #imf.data = mapa[:, bi:bd]
  #imf.writeto(filename)
  
  #Para visualizar las ventanas
     
  plt.imshow(mapa[:,bi:bd], cmap='jet',aspect ='auto', origin='lower',extent=[bi,bd,0,nrb], vmin=2e-19, vmax=2e-16)
  #plt.imshow(mapa[:,bi:bd], aspect ='auto', origin='lower',extent=[bi,bd,0,nrb])

  plt.axvline(x=i,linewidth=0.9,color='white')
  plt.axvline(x=l1,linewidth=0.9,color='white')
  plt.axvline(x=l2,linewidth=0.9,color='white')
  plt.axvline(x=l3,linewidth=0.9,color='white')
  plt.axvline(x=l4,linewidth=0.9,color='white')
  plt.axhline(y=star[dn],linewidth=0.9,color='white') 
  plt.text(i,20,str(nlin+1)+' '+label+' '+str(dn+1)+' '+str(nol+1),color='white')
  plt.show()
